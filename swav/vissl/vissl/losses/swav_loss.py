...
class SwAVLoss(ClassyLoss):
    ...
    def forward(self, output: torch.Tensor, target: torch.Tensor, training_iterations: int):
        self.swav_criterion.use_queue = (
            self.swav_criterion.local_queue_length > 0
            and training_iterations >= self.queue_start_iter
        )
        if not self.was_using_queue and self.swav_criterion.use_queue:
            print(f'Using queue now! global niter = {training_iterations}')
        self.was_using_queue = self.swav_criterion.use_queue
        loss = 0
